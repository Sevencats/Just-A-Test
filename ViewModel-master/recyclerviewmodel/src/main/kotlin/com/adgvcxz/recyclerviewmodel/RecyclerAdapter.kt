...
//        setHasStableIds(true)
    ...
    private fun initItems() {
        ...
        itemClicks()
                .filter {
                    items[it] is LoadingItemViewModel
                }
                .map { LoadMore }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        ...
        return viewMap[viewType]!!.initView(view, parent)  // itemViewHolder
    }

    @Suppress("UNCHECKED_CAST")
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        action?.let {
            holder.itemView.setOnClickListener { _ -> it.onNext(position) }
        }
        if (!isAttachToBind) {
    ...
    @Suppress("UNCHECKED_CAST")
    override fun getItemViewType(position: Int): Int { //itemView的类型
    ...
    @Suppress("UNCHECKED_CAST")
    override fun onViewAttachedToWindow(holder: ItemViewHolder) { //当列表项出现在可视界面时调用
...
//            viewModel.currentModel().items[holder.layoutPosition].action.onNext(WidgetLifeCircleEvent.Attach)
    ...
    override fun onViewDetachedFromWindow(holder: ItemViewHolder) {//做一些解注册和释放资源的工作
...
//            viewModel.currentModel().items[holder.layoutPosition].action.onNext(WidgetLifeCircleEvent.Detach)
    ...
    override fun onAttachedToRecyclerView(recyclerView: RecyclerView) {
        ...
        recyclerView.addOnAttachStateChangeListener(object : View.OnAttachStateChangeListener {
            override fun onViewDetachedFromWindow(v: View?) {  //做一些解注册和释放资源的工作
                if (disposeWhenDetached) {
                    ...
                    disposables.clear() //解除所有订阅者
                    holders.forEach {
                        it.disposables.clear()
                    }
                    (0 until recyclerView.childCount).forEach {
                        val view = recyclerView.getChildAt(it)
                        val holder = (recyclerView.getChildViewHolder(view) as ItemViewHolder)
            ...
            override fun onViewAttachedToWindow(v: View?) {  //当列表项出现在可视界面时调用
